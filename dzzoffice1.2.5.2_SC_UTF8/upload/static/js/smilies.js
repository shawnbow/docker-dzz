...
function smilies_show(id, smcols, seditorkey) {
	...
	if(typeof smilies_type == 'undefined') {
		...
		scriptNode.src = 'data/cache/smilies_var.js?' + VERHASH;
...
function smilies_onload(id, smcols, seditorkey) {
	...
	if(typeof smilies_type == 'object') {
		...
		for(i in smilies_type) {
			...
			if(smilies_type[i][0]) {
				k++;
				smiliestype += '<li ' + (CURRENTSTYPE == key ? 'class="active"' : '') + ' id="'+seditorkey+'stype_'+key+'" onclick="smilies_switch(\'' + id + '\', \'' + smcols + '\', '+key+', 1, \'' + seditorkey + '\');if(CURRENTSTYPE) {$(\''+seditorkey+'stype_\'+CURRENTSTYPE).className=\'\';}this.className=\'active\';CURRENTSTYPE='+key+';doane(event);" ><a href="javascript:;" hidefocus="true">'+smilies_type[i][0]+'</a></li>';
			}
		}
		smiliestype += '</ul></div>';
		if(k<2) smiliestype='';//只有一组表情时不现实分类栏
		$(id).innerHTML = smiliestype + '<div id="' + id + '_data" class="sllt-wrap"></div><div class="sllt_p" id="' + id + '_page"></div>';
...
function seditor_insertunit(key, text, textend, moveend, selappend) {
	...
	if(!isUndefined($(key).selectionStart)) {
		if(selappend) {
			var opn = $(key).selectionStart + 0;
			if(textend != '') {
				text = text + $(key).value.substring($(key).selectionStart, $(key).selectionEnd) + textend;
			}
			$(key).value = $(key).value.substr(0, $(key).selectionStart) + text + $(key).value.substr($(key).selectionEnd);
			if(!moveend) {
				$(key).selectionStart = opn + strlen(text) - endlen;
				$(key).selectionEnd = opn + strlen(text) - endlen;
			}
		} else {
			text = text + textend;
			$(key).value = $(key).value.substr(0, $(key).selectionStart) + text + $(key).value.substr($(key).selectionEnd);
