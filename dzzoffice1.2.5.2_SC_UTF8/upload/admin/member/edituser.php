...
if($do=='add'){
	...
	if(submitcheck('accountadd')){
		...
		if($nickname){
			...
			//处理管理员
			if($_GET['adminid']) C::t('user')->setAdministror($uid,intval($_GET['adminid']));
			...
			showmessage('添加用户成功',ADMINSCRIPT.'?mod=member&op=edituser&uid='.$uid,array('uid'=>$uid));
	...
	if(submitcheck('accountedit')){
		...
		if($weixinid){	
			...
			wx_updateUser($uid);
			//处理管理员
			if($user['adminid']!=intval($_GET['adminid'])) C::t('user')->setAdministror($uid,intval($_GET['adminid']));
			//处理额外空间
			$addsize=intval($_GET['addsize']);
			if(C::t('user_field')->fetch($uid)){
				C::t('user_field')->update($uid,array('addsize'=>$addsize,'perm'=>0));
			...
			C::t('organization_upjob')->insert_by_uid($uid,intval($_GET['upjobid']));
			showmessage('修改用户信息成功',ADMINSCRIPT.'?mod=member&op=edituser&uid='.$uid,array());	
	...
	if(submitcheck('profilesubmit')){
		...
		showmessage('用户资料修改成功',ADMINSCRIPT.'?mod=member&op=edituser&do=profile&uid='.$uid,array());
	}else{
		$allowitems = array();
