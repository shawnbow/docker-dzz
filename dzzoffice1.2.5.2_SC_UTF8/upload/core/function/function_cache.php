...
function updatecache($cachename = '') {
	...
	if(!$updatelist) {
		...
		//处理应用的缓存
		foreach(C::t('app_market')->fetch_all_identifier(1) as $dir) {
			...
			if(is_dir($cachedir)) {
				...
				while($entry = $cachedirhandle->read()) {
					if(!in_array($entry, array('.', '..')) && preg_match("/^cache\_([\_\w]+)\.php$/", $entry, $entryr) && substr($entry, -4) == '.php' && is_file($cachedir.'/'.$entry)) {
						try{
							@include_once DZZ_ROOT.'./dzz/'.$dir.'/cache/cache_'.$entryr[1].'.php';
							//call_user_func('build_cache_'.$dir.'_'.$entryr[1]);
							if(function_exists('build_cache_'.$dir.'_'.$entryr[1])) call_user_func('build_cache_'.$dir.'_'.$entryr[1]);
