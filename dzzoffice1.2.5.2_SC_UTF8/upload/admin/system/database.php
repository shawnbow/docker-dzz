...
if($operation == 'export') {
		...
		if($_GET['method'] == 'multivol') {
			...
			if(trim($sqldump)) {
				...
				if(@!fwrite($fp, $sqldump)) {
					...
					$msg=lang('template','database_export_multivol_redirect',array('volume'=>$volume));
				...
				if($_GET['usezip'] == 1) {
					...
					if(@fwrite($fp, $zip->file()) !== FALSE) {
						...
						$msg.=lang('template','database_export_multivol_succeed', array('volume' => $volume, 'filelist' => $filelist));
						$msg_type='text-success';
					...
					$msg.=lang('template','database_export_zip_succeed', array('filename' => $filename));
					$msg_type='text-success';
			...
			require DZZ_ROOT . './core/config/config.php';
			...
			if(@file_exists($dumpfile)) {
				...
				if($_GET['usezip']) {
					...
					$msg=lang('template','database_export_succeed', array('filename' => $filename));
					$msg_type='text-success';
				}

			} else {
				$msg=lang('template','database_shell_fail');
				$msg_type='text-error';
	...
	if(!submitcheck('deletesubmit')) {
		...
		if(is_dir(DZZ_ROOT.'./data/'.$backupdir)) {
			...
			$msg=lang('template','database_export_dest_invalid');
			$msg_type='text-error';
			include template('database');
			exit();
		}
		...
		$do_import_option=lang('template','do_import_option',array('restore_url'=>$restore_url));
		...
		foreach($exportlog as $key => $val) {
			...
			$info['ftype']=lang('template','db_export_'.$info['type']);
			$info['size'] = sizecount($exportsize[$key]);
			$info['volume'] = count($val);
			$info['method'] = $info['type'] != 'zip' ? ($info['method'] == 'multivol' ? lang('template','db_multivol') : lang('template','db_shell')) : '';
			
			$info['datafile_server'] = '.'.$info['filename'];
			$list[$key]=$info;
			foreach($val as $info) {
				$info['dateline'] = is_int($info['dateline']) ? dgmdate($info['dateline']) : lang('template','unknown');
...
//print_r($list);
		...
		if(is_array($_GET['delete'])) {
			...
			$msg=lang('template','database_file_delete_succeed');
			$msg_type='text-success';
			$redirecturl=dreferer();
		} else {
			$msg=lang('template','database_file_delete_invalid');
			$msg_type='text-error';
			$redirecturl=dreferer();
		}
	}
...
function checkpermission($action, $break = 1) {
	...
	if(!isset($_G['config']['admincp'])) {
		return 'config文件未设置相关此操作的权限，请检查config.php文件中相关admincp的设置';
	} elseif(!$_G['config']['admincp'][$action]) {
		return 'config文件设置不允许此操作，请检查config.php文件中相关admincp的设置';
	} else {
		return true;
	}
}
