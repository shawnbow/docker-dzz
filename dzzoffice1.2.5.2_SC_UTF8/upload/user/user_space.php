...
$uid=intval($_GET['uid']?$_GET['uid']:$_G['uid']);
	...
	if($space['lastvisit']) $profiles['lastvisit']=array('title'=>'最后访问','value'=>dgmdate($space['lastvisit']));
	...
	if(empty($department)) $department='未加入机构或部门';
	...
	$profiles['fusersize']=array('title'=>'空间使用','value'=>$space['fusesize'].' / '.$space['fmaxspacesize']);
