...
class io_ftp extends io_api
{
	...
	public function authorize($refer){
		...
		if(submitcheck('ftpsubmit')){
			...
			$config['on']=1;
			$ftp = new dzz_ftp($config);
			if($ftp->error()) showmessage('FTP 参数设置错误，请检查'.$ftp->error(),dreferer());
			if($ftp->connect()){
				$config['uid']=$uid;
	...
	function getStream($path){
		...
		return $scheme.urlencode($config['username']).':'.urlencode($config['password']).'@'.$config['host'].':'.$config['port'].$arr['path'];
	}
	//获取文件流地址；
	//$path: 路径
