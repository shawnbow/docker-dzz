		...
		if(isset($_G['profile_verifys'][$uid][$fieldid])) {
			if($fieldid=='gender') {
				...
				if($_G['profile_verifys'][$uid][$fieldid]){
					...
					$verifyvalue = implode('-',array_reverse($orgtree));
				}else{
					$verifyvalue = '';
				}
	...
	if($fieldid == 'birthday') {
		...
		if($field['unchangeable'] && $space[$fieldid] > 0) {
			...
			$html=' <script type="text/javascript">'
				  ...
				  ."		if(vals[0].orgid=='other') vals[0].path='无机构和部门';"
				  ...
				  .'	  <input id="sel_'.$fieldid.'"  type="hidden" name="'.$fieldid.'"  value="'.$space[$fieldid].'" />'
				  .'	  <button type="button" id="'.$fieldid.'_Menu" class="btn btn-default dropdown-toggle" data-toggle="dropdown">'
				  .'	'.($space['department_tree']?$space['department_tree']:'请选择机构或部门').' <span class="caret"></span>'
				  .'  </button>'
...
function countprofileprogress($uid = 0) {
	...
	$complete = 0;
	$profile = C::t('user_profile1')->fetch($uid);
