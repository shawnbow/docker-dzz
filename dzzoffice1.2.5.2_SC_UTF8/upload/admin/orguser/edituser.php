...
if($do=='add'){
	...
	if(submitcheck('accountadd')){
		...
		if(!$orgids && $_G['adminid']!=1) showmessage('没有对应部门的权限');
		...
		if($nickname){
			...
			//处理管理员
			if($_GET['adminid'] && $_G['adminid']==1) C::t('user')->setAdministror($uid,intval($_GET['adminid']));
			...
			if($_GET['sendmail']){
				...
				if(!sendmail_cron("$_GET[email] <$_GET[email]>", lang('email', 'email_password_subject'), $email_password_message)) {
					runlog('sendmail', "$_GET[email] sendmail failed.");
				}
			}
			
			showmessage('添加用户成功',ADMINSCRIPT.'?mod=orguser#user_'.$uid,array('uid'=>$uid,'orgids'=>$orgids));
			
	}else{
		$orgid=intval($_GET['orgid']);
		if(!C::t('organization_admin')->ismoderator_by_uid_orgid($orgid,$_G['uid'])){
			exit('<div class="well alert-danger">抱歉！您没有在此机构或部门下添加用户的权限！<br><br>可以在左侧选择有权限的部门，再重试添加</div>');
		}
	...
	if(submitcheck('accountedit')){
		...
		if($weixinid){	
			...
			wx_updateUser($uid);
			//处理管理员
			if($_G['adminid']==1 && $user['adminid']!=intval($_GET['adminid'])) C::t('user')->setAdministror($uid,intval($_GET['adminid']));
			//处理额外空间
			$addsize=intval($_GET['addsize']);
			if(C::t('user_field')->fetch($uid)){
				C::t('user_field')->update($uid,array('addsize'=>$addsize,'perm'=>0));
			}else{
				C::t('user_field')->insert(array('uid'=>$uid,
												 'addsize'=>$addsize,
												 'perm'=>0,
												 'iconview'=>$_G['setting']['desktop_default']['iconview']?$_G['setting']['desktop_default']['iconview']:2,
												 'taskbar'=>$_G['setting']['desktop_default']['taskbar']?$_G['setting']['desktop_default']['taskbar']:'bottom',
		...
		$departs=array();$data_depart=array();
		...
		if($_G['adminid']!=1){
			...
			if(!$uperm) exit('<div class="well alert-danger">抱歉！您没有此用户的管理权限！<br><br>可以在左侧选择有权限管理的用户，再重试添加</div>');
		}
