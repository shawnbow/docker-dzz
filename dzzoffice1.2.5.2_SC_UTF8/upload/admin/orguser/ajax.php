...
if($do=='getchildren'){
	...
	if($_GET['id']=='#'){
		...
		$data[]=array('id'=>'other','text'=>'无机构用户','icon'=>'dzz/system/images/department.png','state'=>array('disabled'=>$disable),"type"=>'default','children'=>true);
	...
	$html='<li role="presentation"><a href="javascript:;" tabindex="-1" role="menuitem" _jobid="0" onclick="selJob(this)">无</a></li>';
	foreach($jobs as $job){
	...
	$borgid=intval($_GET['orgid']);//放在此部门后面
	...
	if(C::t('organization')->update_by_orgid($orgid,array('orgname'=>trim($_GET['text'])))){
		exit(json_encode(array('msg'=>'success')));
	}else{
		exit(json_encode(array('msg'=>'重命名错误')));
	}
}elseif($do=='delete'){	
	...
	if($_GET['type']=='user'){//删除用户
		...
		if($realdelete){
			if($_G['adminid']!=1) exit(json_encode(array('error'=>'没有权限')));
			//判断用户是否在部门中，在部门中的用户不彻底删除
			if(C::t('organization_user')->fetch_orgids_by_uid($uids)){
				exit(json_encode(array('error'=>'在机构或部门中的用户，不支持彻底删除，请先从机构或部门中删除后重试')));
			}
			foreach($uids as $uid){
			//删除用户
			...
			if(C::t('organization_user')->delete_by_uid_orgid($uids,$forgid)){
				...
				exit(json_encode(array('msg'=>'删除错误')));
			}
		}
	...
	if($_GET['type']=='user'){//移动用户
		...
		if(C::t('organization_user')->move_to_by_uid_orgid($uid,$forgid,$orgid,$copy)){
			...
			exit(json_encode(array('error'=>'移动错误')));
		}
	}else{
		$orgid=intval($_GET['orgid']);
	...
	if(C::t('organization_job')->update($jobid,array('name'=>$name))){
		...
		exit(json_encode(array('error'=>'编辑失败')));
	}
}elseif($do=='jobdel'){
	$jobid=intval($_GET['jobid']);
	$orgid=intval($_GET['orgid']);
	...
	if($setarr['id']=C::t('organization_admin')->insert(intval($_GET['uid']),$orgid)){
		...
		exit(json_encode(array('error'=>'添加管理员失败')));
	}
	...
	if(C::t('organization')->setFolderAvailableByOrgid($orgid,intval($_GET['available']))){
		...
		exit(json_encode(array('error'=>'无法设置')));
	}
	...
	if(C::t('organization')->setIndeskByOrgid($orgid,intval($_GET['indesk']))){
		...
		exit(json_encode(array('error'=>'没有开启共享目录，无法设置')));
	}
