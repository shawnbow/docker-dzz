...
class uploadhandler
{
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
           ...
           $file_path = $this->get_upload_path(md5($file->name));
            $append_file = $content_range[1]>0 && $content_range[2]<$content_range[3];
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
	 ...
	 global $_G;
		...
		if($md5 && $attach=DB::fetch_first("select * from %t where md5=%s and filesize=%d",array('attachment',$md5,$filesize))){
			...
			if($attach['aid']=C::t('attachment')->insert($attach,1)){
				...
				if($this->options['tospace']) dfsockopen($_G['siteurl'].'misc.php?mod=movetospace&aid='.$attach['aid'].'&remoteid=0',0, '', '', FALSE, '',1);
				if(in_array(strtolower($attach['filetype']),array('png','jpeg','jpg','gif','bmp'))){
    ...
    public function post($print_response = true) {
		...
		if($this->options['generate_response']){
